...
function deleteTask(index) {
  if (
    confirm(`Are you sure you want to delete this task? "${tasks[index].text}"`)
  ) {
...
function fillTasksContent() {
  let index = 0;
  tasks.forEach((task) => {
    tasksContainer.innerHTML += ` <div class="task" style="background: ${
      task.completed ? "#a7f1a7" : null
    }">
        ...
        <div class="action-buttons">
            <span onclick="toggleCompletionStatus(${index})" id="completed" class="material-symbols-outlined">${
      !task.completed ? "done" : "cancel"
    }</span>
